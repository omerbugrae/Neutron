# VENDORED -- DO NOT EDIT THE CODE BELOW.
#
# Verbatim copy of thrember 0.1.0's features.py ("EMBER feature version 3"),
# upstream SHA-256 aa9b29e2267a037ebec0b234234edfc2f8d2316e579ab1a0fba6878e70288eca
# (44246 bytes). This header block is the only local addition; every line of
# code below is byte-identical to upstream.
#
# Why it lives here instead of being imported from the installed package:
# `import thrember` runs the package's __init__, which pulls in model.py and
# download.py -- the training and dataset-download halves. Those drag in
# polars (176 MB), huggingface_hub and hf_xet, none of which are reachable
# from a scan. Neutron does not train models; it downloads finished ones
# through its own signed Feature Update channel. Vendoring the one module
# that scanning actually needs drops roughly 200 MB from the installer.
#
# The copy must stay byte-identical: the shipped LightGBM models were trained
# against exactly these feature computations, so any edit here silently
# changes what the models are asked to score. To move to a newer thrember,
# replace this file wholesale from the new release, update the version and
# hash above, and re-verify the models -- do not patch it in place.
"""
